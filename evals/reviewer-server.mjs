...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (value === "--cross-source-only") options.crossSourceOnly = true;
    else if (value === "--help") options.help = true;
    else throw new Error(`Unknown argument: ${value}`);
  }
  if (!options.help && !options.arena) throw new Error("Usage: npm run evals:reviewer -- --arena evals/shared-runs/<arena-id> [--port 4174] [--cross-source-only]");
  return options;
}
...
export function buildPairs(candidates, { crossSourceOnly = false } = {}) {
  const pairs = [];
  for (let leftIndex = 0; leftIndex < candidates.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < candidates.length; rightIndex += 1) {
      const first = candidates[leftIndex];
      const second = candidates[rightIndex];
      // Skip same-product matchups so every pair contrasts different subjects,
      // avoiding near-duplicate comparisons between one subject's variants.
      if (crossSourceOnly && first.sourceBundle && second.sourceBundle && first.sourceBundle === second.sourceBundle) continue;
...
function render(){if(!data.pairs.length)return;index=(index+data.pairs.length)%data.pairs.length;const p=current(),saved=answers[p.id]||{},draft=drafts[p.id]||saved;draftChoice=draft.choice||null;const root=document.querySelector('#pair');document.querySelector('#status').textContent=Object.keys(answers).length+' / '+data.pairs.length+' saved · fresh challenger each round';root.innerHTML='<div class="pair-meta">Comparison '+(index+1)+' of '+data.pairs.length+'</div><div class="pair"><article class="option"><div class="option-label">OPTION A</div><div class="script">'+esc(p.left.script)+'</div><button class="choice'+choiceClass('left')+'" data-choice="left">← Option A is better</button></article><article class="option"><div class="option-label">OPTION B</div><div class="script">'+esc(p.right.script)+'</div><button class="choice'+choiceClass('right')+'" data-choice="right">Option B is better →</button></article></div><textarea class="comment" id="comment" placeholder="Why did this one land better? Optional, but great prompt signal.">'+esc(draft.comment||'')+'</textarea><button class="tie'+choiceClass('tie')+'" data-choice="tie">About equal</button><button class="reject'+choiceClass('reject-both')+'" data-choice="reject-both">Reject both — neither is good enough</button><p class="note" id="note"></p><div class="actions"><button class="nav" id="previous">← Previous</button><button class="nav" id="next">Next challenger →</button><button class="save save-choice" id="save-choice">Save choice &amp; next</button></div><p class="keys">Keyboard: ← choose A · → choose B · = tie · X reject both. Your choice does not advance until you save.</p>';document.querySelectorAll('[data-choice]').forEach(b=>b.onclick=()=>selectChoice(b.dataset.choice));document.querySelector('#previous').onclick=()=>move(-1);document.querySelector('#next').onclick=()=>move(1);document.querySelector('#save-choice').onclick=saveChoice;}
function selectChoice(choice){syncDraft();draftChoice=choice;drafts[current().id].choice=choice;document.querySelectorAll('[data-choice]').forEach(button=>button.classList.toggle('selected',button.dataset.choice===choice));document.querySelector('#note').textContent=choice==='reject-both'?'Both candidates will be recorded as rejected training examples. Add why, then save.':'Choice selected — add a comment if useful, then save when ready.'}
function move(offset){syncDraft();index+=offset;render()}
async function saveChoice(){syncDraft();const p=current(),draft=drafts[p.id]||{};const note=document.querySelector('#note');if(!draft.choice){note.textContent='Pick Option A, Option B, About equal, or Reject both before saving.';return}if(draft.choice==='reject-both'&&draft.comment.trim().length<3){note.textContent='Briefly say why both fail before saving a rejection.';return}answers[p.id]={choice:draft.choice,comment:draft.comment||'',reviewedAt:new Date().toISOString()};delete drafts[p.id];await fetch('/api/pairs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({answers})});index+=1;render()}
document.addEventListener('keydown',e=>{if(e.target.tagName==='TEXTAREA')return;if(e.key==='ArrowLeft')selectChoice('left');if(e.key==='ArrowRight')selectChoice('right');if(e.key==='=')selectChoice('tie');if(e.key.toLowerCase()==='x')selectChoice('reject-both')});
...
async function main() {
  ...
  if (options.help) { console.log("Usage: npm run evals:reviewer -- --arena evals/shared-runs/<arena-id> [--port 4174] [--cross-source-only]"); return; }
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && request.url === "/api/pairs") {
        ...
        response.writeHead(200, { "content-type": "application/json" }); response.end(JSON.stringify({ pairs: buildPairs(visibleCandidates(results), { crossSourceOnly: options.crossSourceOnly }), answers: saved.answers || {} })); return;
      }
      ...
      if (request.method === "POST" && request.url === "/api/pairs") {
        ...
        const results = JSON.parse(await readFile(resultsPath, "utf8"));
        const allowed = new Set(buildPairs(visibleCandidates(results), { crossSourceOnly: options.crossSourceOnly }).map((pair) => pair.id));
        const answers = Object.fromEntries(Object.entries(submitted).filter(([id, value]) => allowed.has(id) && ["left", "right", "tie", "reject-both"].includes(value?.choice) && (value.choice !== "reject-both" || String(value.comment || "").trim().length >= 3)).map(([id, value]) => [id, { choice: value.choice, comment: String(value.comment || "").slice(0, 2_000), reviewedAt: value.reviewedAt || new Date().toISOString() }]));
